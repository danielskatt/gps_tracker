...
int module_enqueue_msg(struct module_data *module, void *msg)
{
    ...
    if (err) {
        ...
        /* Purge message queue before reporting an error. This makes sure that the calling module can enqueue and
         *   process new events and is not being blocked by a full message queue.
         *
         * This error is concidered irrecoverable and should be rebooted on.
         */
    ...
    SYS_SLIST_FOR_EACH_CONTAINER_SAFE(&module_list, module, next_module, header){
        if (module->id == id_reg) {
            if (module->supports_shutdown) {
                /* A module shutdown has been registered. Decrease the number of active modules in the application and
                 *   delete the list entry for the corresponding module.
                 */
