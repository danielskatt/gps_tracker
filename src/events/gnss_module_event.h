...
enum gnss_module_event_type {
     ...
     *
     *  The event has associated payload of the type ``struct gnss_module_data`` in the event struct member
     *   ``data.gnss``. All struct members within `data.gnss`` contain valid data.
     *
     *  The format of the location data from the GNSS fix is specified in the event struct member ``data.gnss.format``
     *   and decides how you can access the anonymous union containing the location data.
     *
     *  If the format is ``GNSS_MODULE_DATA_FORMAT_PVT``, the PVT data is available in the event struct member
     *   ``data.gnss.pvt``.
     *
     *  If the format is ``GNSS_MODULE_DATA_FORMAT_NMEA``, the NMEA string is available in the event struct member
     *   ``data.gnss.nmea``.
     */
    GNSS_EVT_DATA_READY,
     ...
     *
     *  The event has associated payload of the type ``struct gnss_module_data`` in the event struct member
     *   ``data.gnss``. The ``data.gnss`` struct members ``search_time`` and ``data.satellites_tracked``
     *  contain valid data.
     */
    GNSS_EVT_TIMEOUT,
    ...
    /** The module has been shut down gracefully. The event has associated payload of the type ``uint32`` in the struct
     *   member
     *  ``data.id``, which contains the module ID used shen acknowledging a shutdown request from the util module.
     */
    GNSS_EVT_SHUTDOWN_READY,

    /** The modem has reported that it needs GPS assistance data. The event has associated payload of the type ``struct
    ...
    /** An error has occurred, and data may have been lost. The event has associated payload of the type ``int`` in the
     *   struct member
     *  ``data.err``, which contains the original error code from that triggered the sending of this event.
     *
     */
    GNSS_EVT_ERROR_CODE,
};
...
/** @brief GNSS module data for associated payload for events of GNSS_EVT_TIMEOUT and GNSS_EVT_DATA_READY types.
 */
